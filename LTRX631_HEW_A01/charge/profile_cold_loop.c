...
void  ProfileCOLD_loop(void)
{
	...
	case StateChgStartPhase2:      /* Beginning of phase 2 */
		...
		if (Memo.BatTemp < 5)
			...
			Memo.Iphase3 = (Memo.BatCap * 10) / 24;
	...
	case StateChgStartPhase3:     /* beginning of phase 3 */
		...
		if (ChgData.TimeMaxPhase3 < 3600L)
			ChgData.TimeMaxPhase3 = 3600L;
		ChgData.TimeMaxPhase3 = ChgData.TimerSecCharge + ChgData.TimeMaxPhase3;
		// Init Loop
		ChgData.TimeLoopPhase = ChgData.TimerSecCharge + (12L * 60L) + 50l;	  // 12min50
		// Init dVdT
		ChgData.ThrlddVdT = -5;   // -5mV/Cells
