...
void  ProfilePZM(void)
{
	...
	case StateChgStartPhase1:     /* Just at beginning of phase 1 */
		...
		Memo.Iphase1 = (Memo.BatCap * 20) / 10;	// 20% Capacity
		Memo.VgazCell = 2505L - (((slong)Memo.BatTemp * 35L) / 10L);

		// Init coef
		ChgData.CoefPhase3 = 120; // Coef = 1.12
	...
	case StateChgPhase1:          /* In phase 1 */
		//if (ChgData.TimerSecCharge > (5*60L))
	...
	case StateChgAvail:
        // MODIF 3.2 : fix bug wih refresh in IDLE mode
        /*if (IdleState != 0)
        {
            CntRefresh = ChgData.TimerSecCharge;
        }
