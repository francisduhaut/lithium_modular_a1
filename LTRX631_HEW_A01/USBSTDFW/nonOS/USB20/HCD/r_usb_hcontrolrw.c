...
*         : 3.26.2012          Function header translation. Ch. tabs to 4 spaces.
*                              SET EDITOR TO INSERT SPACES WHEN TAB PRESSED.
******************************************************************************/

/* $Id: r_usb_hcontrolrw.c 143 2012-05-07 09:16:46Z tmura $ */
...
void usb_hstd_ControlEnd(USB_UTR_t *ptr, uint16_t Status)
{
    ...
    /* PID=NAK & clear STALL */
    usb_cstd_ClrStall(ptr, (uint16_t)USB_PIPE0);
    usb_creg_set_mbw( ptr, USB_CUSE, 1 );
