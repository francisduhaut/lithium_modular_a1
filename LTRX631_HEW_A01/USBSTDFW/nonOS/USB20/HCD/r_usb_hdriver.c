...
/* $Id: r_usb_hdriver.c 143 2012-05-07 09:16:46Z tmura $ */
...
static USB_CB_INFO_t    usb_shstd_ClearStallCall;
...
******************************************************************************/
extern uint16_t    usb_ghstd_ClassRequest[];
...
{
    return (uint8_t *)&usb_ghstd_DeviceDescriptor;
...
{
    return (uint8_t *)&usb_ghstd_ConfigurationDescriptor;
...
Arguments       : USB_UTR_t *ptr : USB internal structure. Selects e.g. channel.
                ...
                : USB_CB_INFO_t callback    : Callback function pointer.
Return          : USB_ER_t          : USB_E_OK etc.
******************************************************************************/
...
void usb_hstd_HcdTask(USB_VP_INT stacd)
{
    ...
    else
    {
        ...
        switch( msginfo)
        {
            ...
            case USB_MSG_HCD_CTRL_END:
                /* Control END */
                usb_hstd_ControlEnd(ptr, ptr->keyword);
                /* Callback */
                (*hp->complete)(ptr, (uint16_t)USB_NO_ARG, (uint16_t)USB_MSG_HCD_CTRL_END);
                /* Release Memory Block */
                usb_hstd_HcdRelMpl(ptr, msginfo);
            break;
            ...
            case USB_MSG_HCD_CLR_STALL:
                ...
                usb_hstd_ClearStall(ptr, pipenum, (USB_CB_t)&usb_hstd_ClearStallResult);
                /* Release Memory Block */
                usb_hstd_HcdRelMpl(ptr, msginfo);
            break;
...
uint16_t usb_hstd_ClearFeature(USB_UTR_t *ptr, uint16_t addr, uint16_t epnum, USB_CB_t complete)
{
    ...
    usb_hstd_TransferStart(&usb_shstd_ClearStallControl);
