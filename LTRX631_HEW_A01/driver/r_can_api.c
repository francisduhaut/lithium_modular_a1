...
/* Max delay waiting for CAN register to flip. It should never timeout. 
Do not set too low. For example if the TxCheck function is not used, and the user 
tries to use the mailbox too soon, the timer may time out since the mailbox is 
occupied. The mailbox will then not send properly. */
 ...
 *******************************************************************************/
uint32_t R_CAN_Create(const uint32_t ch_nr, uint32_t frame_id_mode)
{
	...
	if(SYSTEM.MSTPCRB.LONG & (ch_nr * 2)) /* Check bits 0, 2, or 4 for channel 0, 1, or 2 */
		...
		const uint32_t             frame_type)
{
	...
	if (frame_p->id & 0xF0000000)    /* Check for XID flag bit set in ID field */
	{
...
static uint32_t can_wait_tx_rx(const uint32_t ch_nr, const uint32_t mbox_nr)
{
	...
	if (can_block_p->MCTL[mbox_nr].BIT.TX.TRMREQ)
	{
		...
		if (can_tmo_cnt == 0)
		{
			...
			return FoCanInit(ch_nr, can_block_p->CTLR.BIT.IDFM); // MODIF FD 
		}
	}
...
{    
	...
	if (ch_nr == CH_0)
	{
		...
		can_block_p->BCR.BIT.BRP = CAN0_BRP - 1;
		can_block_p->BCR.BIT.TSEG1 = CAN0_TSEG1 - 1;
		can_block_p->BCR.BIT.TSEG2 = CAN0_TSEG2 - 1;
		can_block_p->BCR.BIT.SJW = CAN0_SJW - 1;
	} 
	else if (ch_nr == CH_1)
	{
		/* Set TSEG1, TSEG2 and SJW. */
		can_block_p->BCR.BIT.BRP = CAN1_BRP - 1;
		can_block_p->BCR.BIT.TSEG1 = CAN1_TSEG1 - 1;
		can_block_p->BCR.BIT.TSEG2 = CAN1_TSEG2 - 1;
		can_block_p->BCR.BIT.SJW = CAN1_SJW - 1;
	}
...
static void config_can_interrupts(const uint32_t ch_nr)
{
	...
	if (ch_nr == 0)
	{
		...
		ICU.IPR[IPR_ICU_GROUPE0].BIT.IPR = CAN0_INT_LVL;

		CAN0.EIER.BYTE = 0xFF; /* enable all error interrupts within peripheral */
	...
	{
#if (USE_CAN_POLL == 0)
		IEN(CAN1, TXM1) = 1; //1 = interrupt enabled.
		...
		ICU.IPR[IPR_ICU_GROUPE0].BIT.IPR = CAN1_INT_LVL;
	...
	if (ch_nr == 2)
	{
#if (USE_CAN_POLL == 0)
		/* Configure CAN Tx interrupt. */
		IEN(CAN2, TXM2) = 1;            //1 = interrupt enabled.
		...
		/* Configure CAN Error interrupt. Must enable group that it belongs to */
		/* in addition to individual source. */
		IEN(ICU, GROUPE0) = 1; //Enable group
		EN(CAN2, ERS2) = 1; //    resolves to:  ICU.GEN[GEN_CAN2_ERS2].BIT.EN2 = 1;

		ICU.IPR[IPR_ICU_GROUPE0].BIT.IPR = CAN2_INT_LVL;

		CAN2.EIER.BYTE = 0xFF; /* Enable all error interrupts within peripheral */
...
void CAN1_RXM0_ISR(void)
{
	/* Use CAN API. */
	uint32_t api_status = R_CAN_OK;

	api_status = R_CAN_RxPoll(CH_1, CANBOX_RX);
	if (api_status == R_CAN_OK)
	{
		CAN1_rx_newdata_flag = 1;
	}
 ...
 *****************************************************************************/
#pragma interrupt    CAN1_ERS1_ISR(vect=VECT_ICU_GROUPE0, enable)

void CAN1_ERS1_ISR(void)
{
	nop();
}/* end CAN1_ERS1_ISR() */
#endif //0
