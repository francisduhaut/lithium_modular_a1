...
/* $Id: r_usb_hreg_abs.c 114 2012-04-09 06:18:09Z tmura $ */
...
******************************************************************************/
#pragma    section    _hreg_abs

/******************************************************************************
Function Name   : usb_hstd_SetHubPort
Description     : No processing because this function is prepared for R8A66597(ASSP)
Arguments       : USB_UTR_t *ptr   ; Not used
                : uint16_t addr    ; Not used
                : uint16_t upphub  ; Not used
                : uint16_t hubport ; Not used
Return value    : none
******************************************************************************/
...
/******************************************************************************
Function Name   : usb_hstd_INITConnect2
Description     : Checks the connect state to USB PORT0 and USB PORT1
Arguments       : USB_UTR_t *ptr   ; USB system internal structure. Selects channel.
Return value    : none
******************************************************************************/
void usb_hstd_INITConnect2(USB_UTR_t *ptr)
{
    uint16_t        connect_inf0, connect_inf1;

    /* Check connection */
    connect_inf0 = usb_hstd_INITConnect0(ptr, USB_PORT0);
    connect_inf1 = usb_hstd_INITConnect0(ptr, USB_PORT1);

    if((connect_inf0 == USB_DETACH)&&(connect_inf1 == USB_DETACH))
    {
        /* Check clock */
        usb_hstd_ChkClk(ptr, (uint16_t)0, (uint16_t)USB_DETACHED);
    }

} /* eof of usb_hstd_INITConnect2 */
...
void usb_hstd_DetachProcess(USB_UTR_t *ptr, uint16_t port)
{
    ...
    usb_hstd_OvrcrDisable( ptr, port);
